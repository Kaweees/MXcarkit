...
class PIDcontroller(Node):
    def __init__(self, model_path: str):
        ...
        self.ack_cmd_publisher = self.create_publisher(
        ...
        self.targets = {
            lbl: {
                ...
                "history": deque(
                    [False] * self.len_history, maxlen=self.len_history
                ),  # Detection history
        ...
        self.get_logger().info(
            f"Parsing predictions for {self.id2target} and {self.center_line_id}:center"
        )
    ...
    def waypoint_callback(self, msg: PoseStamped):
        ...
        if np.isnan(point).any():
            ...
            if any(self.line_found):
                ...
                ackermann_msg = to_ackermann(
                    self.speed, self.last_steering_angle, timestamp_unix
                )
            else:
                # Line hasn't been detected in self.max_not_found frames, stop the vehicle
                ackermann_msg = to_ackermann(
                    0.0, self.last_steering_angle, timestamp_unix
                )
    ...
    def stop_and_wait(self, target_data, duration=2.0):
        ...
        target_data["detected"] = (
            True
            if np.mean(target_data["history"]) > target_data["threshold"]
            else False
        )
    ...
    def change_speed(self, target_data, speed_limit):
        ...
        if target_data["detected"] and not target_data["reacted"]:
            if target_data["distance"] < target_data["min_distance"]:
                ...
                self.get_logger().info(
                    f"Changing speed for {speed_limit} mph speed limit sign"
                )
